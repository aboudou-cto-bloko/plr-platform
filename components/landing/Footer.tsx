...
export function Footer() {
  return (
    <footer className="relative overflow-hidden bg-[#020810]">
      {/* Top separator — contraste net avec FinalCTA */}
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/[0.12] to-transparent" />

      {/* Subtle radial glow */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_bottom,_var(--tw-gradient-stops))] from-primary/[0.06] via-transparent to-transparent" />
            ...
            <div className="mt-6 flex items-center gap-2">
              {[
                ...
                <Link
                  ...
                  className="flex h-8 w-8 items-center justify-center rounded-full border border-white/[0.08] bg-white/[0.04] text-white/35 transition-all hover:border-primary/30 hover:bg-primary/10 hover:text-primary"
                >
                  <svg
                    className="h-3.5 w-3.5"
                    fill={social.isFilled ? "currentColor" : "none"}
                    stroke={social.isFilled ? undefined : "currentColor"}
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                  >
                    {social.icon}
  ...
  <div className="flex items-center gap-1.5 rounded-full border border-white/[0.06] bg-white/[0.02] px-3 py-1.5">
    ...
    <span className="text-[11px] text-white/25">Plateforme opérationnelle</span>
  </div>;
}
